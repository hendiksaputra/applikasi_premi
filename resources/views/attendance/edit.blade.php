              ...
              <form action="{{ url('attendances/' . $attendance->id) }}" method="post">
                  ...
                  <select data-placeholder="Choose an employee..."
                    ...
                    @foreach ($employees as $item)
                      <option value="{{ $item->id }}"
                        {{ old('employee_id', $attendance->employee_id) == $item->id ? 'selected' : null }}>
                        {{ $item->nik }} -
                        {{ $item->name }}</option>
                    @endforeach
                  </select>
                ...
                <div class="form-group">
                  ...
                  <input type="datetime-local" name="present_time"
                    class="form-control @error('present_time') is-invalid @enderror"
                    value="{{ old('present_time', date('Y-m-d\TH:i', strtotime($attendance->present_time))) }}">
                  @error('present_time')
                    <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
                <div class="form-group">
                  ...
                  <select name="attendance_category_id"
                    ...
                    @foreach ($category as $item)
                      <option value="{{ $item->id }}"
                        {{ old('attendance_category_id', $attendance->attendance_category_id) == $item->id ? 'selected' : null }}>
